...
public class Table {
     ...
     */
    public Table(GeneralMemoryStub m) {
        mem = m;
    ...
    public synchronized void saluteTheClient() throws MyException {
        ((Waiter)Thread.currentThread()).setWaiterState("Appraising_Situation");
    ...
    public synchronized int enter() throws MyException {
        ((Student)Thread.currentThread()).setWaiterState("Appraising_Situation");
        mem.studentLog("Appraising_Situation");
        atTable = atTable + 1;
        int pos = atTable;
        menu=true;
        try {
